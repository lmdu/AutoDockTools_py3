...
class pKaRoutines:
    ...
    def get_interaction_energies(self, pKa_center, titration, state):
        ...
        print(
            'In get_interaction_energies. center: group: %s, titration: %s, state: %s' % (pKa_center, titration, state))
        #
        ...
        for pKa in self.pKas:
            ...
            for titration in pKa.pKaGroup.DefTitrations:
                for state in titration.allstates:
                    print('%10s\t%5.3f\t\t%5.3f\t\t%5.3f' % (
                    state, titration.modelpKa, pKa.desolvation[state], pKa.background[state]))
        print()
        print()
        #
            ...
            for titration in pKaGroup.DefTitrations:
                ...
                for state in all_states:
                    if self.is_charged(pKa, titration, state) == 1:
                        ...
                        print('Energy difference for %s -> %s [reference state] is %5.2f pKa units' % (
                        state, ref_state, intpKa))
        ...
        for chain in protein.getChains():
            for residue in chain.get("residues"):
                for atom in residue.get("atoms"):
                    ...
                    print('%2s %4s %3d %4s %5.2f %5.2f' % (
                    chain.chainID, residue.name, residue.resSeq, atomname, charge, radius))
